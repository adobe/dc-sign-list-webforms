...
This Java-based command-line tool provides a simple method to list all the webforms within an [Adobe Acrobat Sign](https://www.adobe.com/sign.html) account.

## Set-Up Instructions

+ Download the latest release of the `aas-list-webforms-<version>.jar` from the [Releases](https://git.corp.adobe.com/sign-acs/aas-list-webforms/releases) page
...
5. Create a key with the scopes needed (`user_read` and `widget_read` is required for your account)
...
```sh
java -jar aas-list-webforms-<version>.jar <integrationKey> [--sandbox]
```
...
```sh
java -jar aas-list-webforms-<version>.jar <integrationKey> > output.csv
```
